...
#вариант для словарей где значения на отдельных строках с 1.
import codecs
import re
import sys
...
anydigit2 = re.compile("\d+\.")
roman = re.compile(r'\]([IV]+)\[')
...
#                            print (polysemic)
                    ...
                    if newentry:
                        if options.abbrev:
                            line = remove_stopwords(line)
                        if "[m2]" in line and not "[ex]" in line:
                            line = re.sub("\[.*?\]", "", line).strip()
                            line = clear_text(line)
                            if  len(line) > 2:
                                polysemic[newentry].append(line)
